...
client.on('ready', async () => {
 ...
 client.user.setActivity(`Hayunn ♥ Ponçik`, { type:'WATCHING' })
      ...
      if (msg.content == caps) {
        if (!msg.member.permissions.has("ADMINISTRATOR")) {
          if (!msg.mentions.users.first()) {
            msg.delete();
            return msg.channel.send(`:x: ${msg.member}, __**Bu sunucuda \`Caps\` korumusı aktif !**__  `).then(nordx => nordx.delete({timeout: 5000}))
          ...
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                      ...
                      return msg.reply('Bu sunucuda \`Küfür\` engelleme aktif!').then(nordx => nordx.delete({timeout: 5000}))
            }              
          } catch(err) {
            console.log(err);
          }
 ...
 if (veri) {
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.reply(':x: __**Yakaladım Seni! Reklam Yasak.**__').then(nordx => nordx.delete({timeout: 5000}))                          
...
client.on("guildMemberAdd", member => {
  ...
  moment.locale("tr")
   ...
   var rol = member.guild.roles.cache.get("790202893227524123") //Cezalı Rol İD
   var kayıtsız = member.guild.roles.cache.get("790202893227524124") //Alınacak Rol İD
